...
def vote_on_object(
    ...
    try:
        ...
        raise Http404("No {} found for {}.".format(model._meta.app_label, lookup_kwargs))
...
def vote_on_object_with_lazy_model(request, app_label, model_name, *args, **kwargs):
    ...
    if not model:
        return HttpResponseBadRequest(
            "Model {}.{} does not exist".format(app_label, model_name)
        )
...
def xmlhttprequest_vote_on_object(
    ...
    try:
        ...
        return json_error_response(
            "No {} found for {}.".format(model._meta.verbose_name, lookup_kwargs)
        )
